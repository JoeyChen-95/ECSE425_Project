...
BEGIN
	...
	BEGIN
		...
		IF (clock'event AND clock = '1') THEN
			IF (memwrite = '1') THEN
				ram_block(to_integer(unsigned(address))) <= writedata(7 downto 0);
                ram_block(to_integer(unsigned(address))+1) <= writedata(15 downto 8);
                ram_block(to_integer(unsigned(address))+2) <= writedata(23 downto 16);
                ram_block(to_integer(unsigned(address))+3) <= writedata(31 downto 24);
			END IF;
